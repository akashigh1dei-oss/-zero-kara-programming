...
function selectQuizAnswer(choice){
 if(quizAnswers.length>quizIndex)return;
...
function finishQuiz(){
 ...
 document.getElementById('quizNext').hidden=true;document.getElementById('quizRetry').hidden=false;
...
document.getElementById('quizNext').addEventListener('click',()=>{if(quizAnswers.length!==quizIndex+1)return;if(quizIndex===quizQuestions.length-1)finishQuiz();else{quizIndex++;renderQuizQuestion();}document.getElementById('quizQuestion').scrollIntoView({block:'center'});});
